...
uint8_t SensorBusModule::receiveMsg(uint8_t* buf, uint8_t maxBufLen) {
	_disableInterrupts();
	_strobeClk();
	// The following will get the first byte for the message, which is the
	// message length, or it will get a negative value indicating an error.
	int8_t msgLen = _receiveByte();
	if (msgLen > 0 && (msgLen <= SB_MSG_BUF_MAX_LEN)) {
		buf[0] = msgLen;
		// Now get the rest of the message
		uint8_t idx = 1;
		while (idx < msgLen) {
			int8_t inByte = _receiveByte();
			if (inByte >= 0) {
				buf[idx] = inByte;
				idx++;
			} else {
...
uint8_t SensorBusModule::sendMessage(uint8_t* msgBuf) {
	...
	_disableInterrupts(); // Disable interrupts on DAT
...
void SensorBusModule::_disableInterrupts(void) {
	_pinCtrlBase[*_datCtrl] &= ~PORT_ISC_gm;  			// Clear ISC bits
	_pinCtrlBase[*_datCtrl] |= PORT_ISC_INTDISABLE_gc;  // Disable interrupts
}

/**
 @retval uint8_t value of received byte or negative value indicating an error
 */
int8_t SensorBusModule::_receiveByte(void) {
	int8_t inByte = 0;
	uint8_t bitIdx = 0;
	bool error = false;
	while (bitIdx < 8 && !error) {
		// wait for clk LOW
		bool clkReady = _waitForState(_port, _clk, LOW, 0xFFFF, 10);
		if (clkReady) {
			// read bit
			if (_port->IN & _dat) { // pin is HIGH
				inByte |= (1 << bitIdx);
			}
			// wait for clk HIGH
			bool clkHigh = _waitForState(_port, _clk, HIGH, 0xFFFF, 10);
			if (clkHigh) {
				bitIdx++;
			} else {
				inByte = SB_ERR_CLK_WAIT_TO;
				error = true;
			}
		} else {
			inByte = SB_ERR_CLK_WAIT_TO;
...
void SensorBusModule::_strobeClk(void) {
	_port->DIRSET = _clk;
	_port->OUTCLR = _clk;
