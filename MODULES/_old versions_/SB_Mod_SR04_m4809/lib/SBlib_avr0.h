...
public:
	...
	void init(void);
	uint8_t recvMsgBuf[SB_MSG_BUF_MAX_LEN];
	uint8_t sendMessage(uint8_t* msgBuf);
	uint8_t receiveMsg(uint8_t* buf, uint8_t maxBufLen);
...
protected:
	...
	// METHODS
	void _disableInterrupts(void);
	int8_t _receiveByte(void);
	void _setDefaults();
	void _strobeClk(void);
